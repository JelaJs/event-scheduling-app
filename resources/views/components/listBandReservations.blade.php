        ...
        @foreach ($band->reservations as $reservation)
            <tr class="border-b hover:bg-gray-100">
                ...
                <td class="px-6 py-4 text-center">
                    @if($reservation->band_status == 'pending')
                        <a href="{{route('manager.band.status', [$reservation->id, 'accepted'], )}}" class="px-4 py-2 text-white bg-blue-500 rounded-lg hover:bg-blue-600">Accept</a>
                    @endif

                    @if($reservation->band_status == 'accepted')
                        <p>Accepted</p>
                    @endif
                </td>
                <td class="px-6 py-4 text-center">
                    @if($reservation->band_status == 'pending')
                        <a href="{{route('manager.band.status', [$reservation->id, 'rejected'], )}}" class="px-4 py-2 text-white bg-red-500 rounded-lg hover:bg-red-600">Reject</a>
                    @endif

                    @if($reservation->band_status == 'rejected')
                        <p>Rejected</p>
                    @endif
                </td>
            </tr>
